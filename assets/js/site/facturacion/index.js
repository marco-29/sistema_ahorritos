...
// Configuraciones
(actual_url.indexOf("index") < 0) ? method_call = "facturacion/" : method_call = "";
$.fn.dataTable.ext.errMode = 'throw'; // Configuración de manejo de errores de DataTables

$(document).ready(function () {
    obtener_opciones_select_regimen_fiscal(); // Obtención de opciones para el select 'regimen_fiscal'
    obtener_opciones_select_regimen_fiscal_moral(); // Obtención de opciones para el select 'regimen_fiscal'
    obtener_opciones_select_uso_cfdi(); // Obtención de opciones para el select 'uso_cfdi'
    obtener_opciones_select_uso_cfdi_moral(); // Obtención de opciones para el select 'uso_cfdi'
    obtener_opciones_select_estatus_factura();
    obtener_opciones_select_pago_identificador(); // Obtencion de opciones para el select 'pago identificador'
    ...
    $('#table').on('dblclick', 'td.editable-cell', function () {
        if (!flag_editando) {
            ...
            } else if (columna_nombre === "pago_identificador") {
                var input = generar_campo_de_celda_a_editar('select', valor_original_de_celda, select_pago_identificador);
            } else {
...
function createEditableCells(row, data, dataIndex) {
    var columnsToEdit = [4, 5, 6, 7, 8, 9, 10, 12, 13];
...
// Función para obtener opciones del select 'pago_identificador'
async function obtener_opciones_select_pago_identificador() {
    // Realizar una solicitud AJAX para obtener las opciones de select_pago_identificador
    $.ajax({
        url: method_call + "obtener_opciones_select_pago_identificador",
        method: 'GET',
        dataType: 'json',
        success: function (data) {
            select_pago_identificador = data;
            console.log('Opciones de pago_identificador cargadas:', select_pago_identificador);
        },
        error: function (xhr, status, error) {
            console.error('Error al obtener opciones de pago_identificador: ' + error);
        }
    });
}

function subir_archivos(factura_identificador, nombre_desarrollo, nombre_inmueble, pago_identificador, concepto, cliente_nombre, monto, rfc, codigo_postal, regimen_fiscal, uso_cfdi) {
    ...
    document.getElementById("regimen_fiscal").innerHTML = regimen_fiscal;
    document.getElementById("cfdi").innerHTML = uso_cfdi;
}
