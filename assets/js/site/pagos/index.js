...
$(document).ready(function () {
    ...
    table = $('#table').DataTable({
        ...
        "columns": [
            { "data": "id" },
            { "data": "identificador" },
            { "data": "inmueble_identificador" },
            { "data": "proceso_venta_identificador" },
        ...
        rowCallback: function (row, data, index) {
            ...
            if (data["estatus_pago"] == "Cobrado") {
                $(row).find('td:eq(10)').css('background-color', '#37BC9B');
                ...
                $(row).find('td:eq(10)').css('background-color', '#f08383');
                $(row).find('td:eq(10)').css('color', 'white');
            }
        },
        'language': {
    ...
    function guardar_valor_de_celda(celda_seleccionada, columna_nombre, input) {
        ...
        $.ajax({
            url: method_call + "actualizar_pago", // Actualiza con la ruta correcta
            method: 'POST',
            data: {
...
async function obtener_saldos_del_mes() {

    document.getElementById('mensaje_proceso_venta').innerHTML = '<i class="fa fa-spinner spinner"></i>&nbsp;Espere…';

    var proceso_venta_respuesta = await fetch(method_call + "obtener_saldos");
...
function createEditableCells(row, data, dataIndex) {
    var columnsToEdit = [4, 5, 6, 7, 10]; // Índices de columnas editables
